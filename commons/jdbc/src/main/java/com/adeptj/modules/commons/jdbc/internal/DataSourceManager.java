...
public class DataSourceManager {
    ...
    HikariDataSource getDataSource(String name) {
        return this.hikariDataSources.get(name);
    }

    void closeDataSource(String name) {
        HikariDataSource dataSource = this.hikariDataSources.remove(name);
        if (dataSource == null) {
            LOGGER.warn("HikariDataSource: [{}] not found!!", name);
        } else {
            try {
                dataSource.close();
