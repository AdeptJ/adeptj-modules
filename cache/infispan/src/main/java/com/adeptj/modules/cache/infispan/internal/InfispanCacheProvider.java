 ...
 */
@Component(immediate = true, ds = false, metatype = true, configurationFactory = true, name = InfispanCacheProvider.SERVICE_PID, 
...
public class InfispanCacheProvider implements CacheProvider, ManagedServiceFactory {

	private static final Logger LOGGER = LoggerFactory.getLogger(InfispanCacheProvider.class);
	...
	public InfispanCacheProvider(CacheManager cacheMgr) {
			...
			try {
				for (Entry<String, CacheConfig> entry : this.configMap.entrySet()) {
					...
					if (StringUtils.equals(name, cacheName)) {
						...
						cache = new InfispanCache<>(ehCache);
